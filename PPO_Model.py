...
class PPO(ActorCriticRLModel):
    def __init__(self, policy, env, gamma=0.99, n_steps=2048, ent_coef=0.01, learning_rate=3e-4, vf_coef=0.5,
                 max_grad_norm=0.5, lam=0.95, nminibatches=8, noptepochs=4, cliprange=0.2, cliprange_vf=None,
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            # Checks if the input policy for the PPO model is an instance of common.policies.ActorCriticPolicy
            assert issubclass(self.policy, ActorCriticPolicy), "Error: the input policy for the PPO model must be " \
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    if self.clip_range_vf_ph is None:
                        # No clipping if clip range vf ph is none
                    ...
                    # Calculate Value Function losses
                    vf_losses1 = tf.square(vpred - self.rewards_ph)
                    vf_losses2 = tf.square(vpred_clipped - self.rewards_ph)
                    self.vf_loss = .5 * tf.reduce_mean(tf.maximum(vf_losses1, vf_losses2))

                    # Calculate Policy Gradient losses
                    # neglogpac = negative log probabilities
                    ...
                    # Calculate approximate kl divergence between old and new policy
                ...
                # Create summaries for tensorboard log
    ...
    # Returns the policy loss, value loss, entropy,
    # approximate kl divergence between old and new policy, and clipfrac (clip factor)
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO_Sonic",
...
# Runs the model once it has been trained
